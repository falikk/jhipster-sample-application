...
/**
 * Integration tests for the {@link MetaStarMappingResource} REST controller.
 */
@IntegrationTest
@AutoConfigureMockMvc
@WithMockUser
class MetaStarMappingResourceIT {
    ...
    private static final String ENTITY_API_URL = "/api/meta-star-mappings";
    private static final String ENTITY_API_URL_ID = ENTITY_API_URL + "/{id}";

    @Autowired
    private MetaStarMappingRepository metaStarMappingRepository;

    @Autowired
    private EntityManager em;

    @Autowired
    private MockMvc restMetaStarMappingMockMvc;

    private MetaStarMapping metaStarMapping;
     ...
     */
    public static MetaStarMapping createEntity(EntityManager em) {
        MetaStarMapping metaStarMapping = new MetaStarMapping()
            .name(DEFAULT_NAME)
     ...
     * if they test an entity which requires the current entity.
     */
    public static MetaStarMapping createUpdatedEntity(EntityManager em) {
        MetaStarMapping metaStarMapping = new MetaStarMapping()
            .name(UPDATED_NAME)
    ...
    public void initTest() {
        metaStarMapping = createEntity(em);
    }

    @Test
    @Transactional
    void getAllMetaStarMappings() throws Exception {
        // Initialize the database
        metaStarMapping.setId(UUID.randomUUID().toString());
        metaStarMappingRepository.saveAndFlush(metaStarMapping);

        // Get all the metaStarMappingList
        restMetaStarMappingMockMvc
            .perform(get(ENTITY_API_URL + "?sort=id,desc"))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(metaStarMapping.getId())))
    ...
    @Test
    @Transactional
    void getMetaStarMapping() throws Exception {
        // Initialize the database
        metaStarMapping.setId(UUID.randomUUID().toString());
        metaStarMappingRepository.saveAndFlush(metaStarMapping);

        // Get the metaStarMapping
        restMetaStarMappingMockMvc
            .perform(get(ENTITY_API_URL_ID, metaStarMapping.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.id").value(metaStarMapping.getId()))
    ...
    @Test
    @Transactional
    void getNonExistingMetaStarMapping() throws Exception {
        // Get the metaStarMapping
        restMetaStarMappingMockMvc.perform(get(ENTITY_API_URL_ID, Long.MAX_VALUE)).andExpect(status().isNotFound());
    }
}
