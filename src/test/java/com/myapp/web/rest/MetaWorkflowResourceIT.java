...
/**
 * Integration tests for the {@link MetaWorkflowResource} REST controller.
 */
@IntegrationTest
@AutoConfigureMockMvc
@WithMockUser
class MetaWorkflowResourceIT {
    ...
    private static final String ENTITY_API_URL = "/api/meta-workflows";
    private static final String ENTITY_API_URL_ID = ENTITY_API_URL + "/{id}";

    @Autowired
    private MetaWorkflowRepository metaWorkflowRepository;

    @Autowired
    private EntityManager em;

    @Autowired
    private MockMvc restMetaWorkflowMockMvc;

    private MetaWorkflow metaWorkflow;
     ...
     */
    public static MetaWorkflow createEntity(EntityManager em) {
     ...
     * if they test an entity which requires the current entity.
     */
    public static MetaWorkflow createUpdatedEntity(EntityManager em) {
        MetaWorkflow metaWorkflow = new MetaWorkflow()
            .name(UPDATED_NAME)
    ...
    public void initTest() {
        metaWorkflow = createEntity(em);
    }

    @Test
    @Transactional
    void getAllMetaWorkflows() throws Exception {
        // Initialize the database
        metaWorkflow.setId(UUID.randomUUID().toString());
        metaWorkflowRepository.saveAndFlush(metaWorkflow);

        // Get all the metaWorkflowList
        restMetaWorkflowMockMvc
            .perform(get(ENTITY_API_URL + "?sort=id,desc"))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(metaWorkflow.getId())))
    ...
    @Test
    @Transactional
    void getMetaWorkflow() throws Exception {
        // Initialize the database
        metaWorkflow.setId(UUID.randomUUID().toString());
        metaWorkflowRepository.saveAndFlush(metaWorkflow);

        // Get the metaWorkflow
        restMetaWorkflowMockMvc
            .perform(get(ENTITY_API_URL_ID, metaWorkflow.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.id").value(metaWorkflow.getId()))
    ...
    @Test
    @Transactional
    void getNonExistingMetaWorkflow() throws Exception {
        // Get the metaWorkflow
        restMetaWorkflowMockMvc.perform(get(ENTITY_API_URL_ID, Long.MAX_VALUE)).andExpect(status().isNotFound());
    }
}
